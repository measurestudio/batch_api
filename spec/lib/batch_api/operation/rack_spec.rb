...
describe BatchApi::Operation::Rack do
  ...
  let(:operation) { described_class.new(op_params, env, app) }
  let(:app) { instance_double(Sinatra::Base, call: [200, {}, ['foo']]) }
  ...
  describe '#initialize' do
    %w[method url params headers].each do |a|
      attr = a
    ...
    it 'defaults method to GET if not provided' do
      op = described_class.new(op_params.except('method'), env, app)
      expect(op.method).to eq('GET')
    end

    it 'defaults params to {} if not provided' do
      op = described_class.new(op_params.except('params'), env, app)
      expect(op.params).to eq({})
    end

    it 'defaults headers to {} if not provided' do
      op = described_class.new(op_params.except('headers'), env, app)
    ...
    it 'does a deep dup of the env' do
      ...
      operation.env.to_a.flatten.each_with_index do |obj, index|
        ...
        expect(obj.object_id).not_to eq(flat_env[index].object_id) if obj.is_a?(Hash) || obj.is_a?(Array)
      end
    end
    ...
    it 'raises a MalformedOperationError if URL is missing' do
      ...
      expect do
        described_class.new(no_url, env, app)
      end.to raise_exception(BatchApi::Errors::MalformedOperationError)
    end
  end

  describe '#process_env' do
    let(:processed_env) { operation.tap(&:process_env).env }
    ...
    it 'updates the rack query string' do
      ...
      expect(processed_env[key]).not_to eq(env[key])
      expect(processed_env[key]).to eq(op_uri.query)
    end
    ...
    context 'with a query_hash' do
      it 'sets it to params for a GET' do
        operation.method = 'GET'
        processed_env = operation.tap(&:process_env).env
        get_params = Rack::Utils.parse_nested_query(op_uri.query).merge(op_params['params'])
    ...
    context 'when it works' do
      let(:result) do
        [
          200,
          { header: 'footer' },
          instance_double(Rack::Response, body: '{"data":2}'),
        ]
      end
      ...
      it 'returns a BatchAPI::Response made from the result' do
        ...
        allow(BatchApi::Response).to receive(:new).with(result).and_return(response)
      ...
      it 'returns a BatchApi::Response from an ErrorWrapper for errors' do
        ...
        rendered = double
        response = double
        b_err = instance_double(BatchApi::ErrorWrapper, render: rendered)

        # simulate the error
        allow(app).to receive(:call).and_raise(err)
        # we'll create the BatchError
        allow(BatchApi::ErrorWrapper).to receive(:new).with(err).and_return(b_err)
        # render that as the response
        allow(BatchApi::Response).to receive(:new).with(rendered).and_return(response)
